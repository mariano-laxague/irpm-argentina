  ...
  <div class="data-alternative">
    ...
    <div class="data-table-wrap">
      <table class="data-table">
        <caption class="sr-only">Últimas diez observaciones de IRPM y series auxiliares</caption>
        <thead><tr><th>Fecha</th><th>IRPM</th><th>Suavizado</th><th>Ajuste VIX</th></tr></thead>
...
function initDataAlternative() {
  ...
  summary.textContent = `Última observación: ${latest.d}. IRPM ${latest.v.toFixed(1)}; ` +
    `suavizado ${latest.s == null ? 'no disponible' : latest.s.toFixed(1)}; ` +
    `ajuste VIX ${latest.a == null ? 'no disponible' : latest.a.toFixed(1)}.`;
  body.innerHTML = ALL_DATA.slice(-10).reverse().map(d => `<tr><td>${d.d}</td><td>${d.v.toFixed(1)}</td>` +
    `<td>${d.s == null ? '—' : d.s.toFixed(1)}</td><td>${d.a == null ? '—' : d.a.toFixed(1)}</td></tr>`).join('');
  document.getElementById('download-csv')?.addEventListener('click', () => {
    const rows = ['fecha,irpm,suavizado,ajuste_vix', ...ALL_DATA.map(d =>
      [d.d, d.v, d.s ?? '', d.a ?? ''].join(','))];
