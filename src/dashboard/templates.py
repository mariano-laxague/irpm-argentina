"""Plantillas est?ticas del dashboard IRPM; separadas del acceso a datos."""
...
.skip-link {
  ...
  background: #fff; color: #1c1917; padding: 8px 12px; border-radius: 6px;
...
.what-caveat { margin-top: 18px; font-size: 12px; line-height: 1.55; color: #78716c; }
.what-live-label-top {
...
.range-handle:focus-visible { outline: 3px solid rgba(180,83,9,.35); outline-offset: 3px; }
.range-labels {
  display: flex;
  justify-content: space-between;
  font-size: 10px;
...
.data-section {
  min-height: calc(100vh - 54px); padding: 44px 40px 24px; overflow: visible; background: #faf7f2;
}
.data-alternative { max-width: 980px; margin: 20px auto 0; background: #fff; border: 1px solid #e8e0d5; border-radius: 12px; padding: 22px; }
.data-summary { color: #57534e; font-size: 14px; line-height: 1.6; margin-bottom: 16px; }
.data-actions { display: flex; align-items: center; justify-content: space-between; gap: 14px; margin-bottom: 14px; }
.data-actions h3 { font-size: 16px; }
.data-download { border: 1px solid #b45309; border-radius: 6px; background: #fff; color: #92400e; padding: 7px 10px; font: inherit; font-size: 12px; font-weight: 600; cursor: pointer; }
.data-table-wrap { overflow-x: auto; }
.data-table { width: 100%; border-collapse: collapse; font-size: 13px; }
.data-table th, .data-table td { padding: 8px 10px; text-align: left; border-top: 1px solid #e8e0d5; white-space: nowrap; }
.data-table th { color: #57534e; font-size: 11px; text-transform: uppercase; letter-spacing: .04em; }
...
<h1 class="sr-only">IRPM: Índice de Riesgo Político del Mercado</h1>
  ...
  <div class="what-card">
    ...
    <div class="what-inner">
      ...
      <div class="what-right">
        <div class="what-text">
          <p>
            El IRPM resume cómo están valuando los mercados las condiciones financieras argentinas vinculadas al
            escenario político y económico. Combina mercado cambiario, deuda soberana y volatilidad accionaria en
            una sola lectura comparable en el tiempo.
          </p>
          <p>
            <strong>Cómo leerlo:</strong> 100 es la referencia de diciembre de 2023. Por encima de 100, las señales
            incluidas están, en conjunto, más favorables que esa base; por debajo, muestran mayor presión o fragilidad.
            El cambio mensual muestra si esa lectura se está fortaleciendo o deteriorando.
          </p>
        </div>
        <div class="what-pills">
          <span class="what-pill">Hoy: __IEP_NOW__, levemente por encima de la base</span>
          <span class="what-pill">Sube si mejoran las tres señales; baja si se tensionan</span>
          <span class="what-pill">Sirve para detectar cambios de clima financiero</span>
        </div>
        <p class="what-caveat">No predice elecciones ni prueba por sí solo la causa de un movimiento: aporta una señal de mercado que conviene contrastar con el contexto.</p>
      </div>
    </div>
  </div>
</section>
...
<section class="chart-section slide" id="evolucion">
  ...
  <div class="range-selector-wrap">
    <div class="range-track" id="range-track">
      ...
      <button class="range-handle" id="range-handle-left" data-side="left" type="button"
        role="slider" aria-label="Inicio del período" aria-valuemin="0" aria-valuemax="100"></button>
      <button class="range-handle" id="range-handle-right" data-side="right" type="button"
        role="slider" aria-label="Fin del período" aria-valuemin="0" aria-valuemax="100"></button>
    </div>
    <div class="range-labels" id="range-labels" aria-live="polite"></div>
  </div>
...
<section class="data-section slide" id="datos" aria-labelledby="datos-title">
  <div class="section-eyebrow">Alternativa al gráfico</div>
  <div class="section-title" id="datos-title">Datos del IRPM en texto y tabla</div>
  <div class="data-alternative">
    <p class="data-summary" id="data-summary"></p>
    <div class="data-actions">
      <h3>Últimas 10 observaciones</h3>
      <button class="data-download" id="download-csv" type="button">Descargar serie CSV</button>
    </div>
    <div class="data-table-wrap">
      <table class="data-table">
        <caption class="sr-only">Últimas diez observaciones del IRPM</caption>
        <thead><tr><th>Fecha</th><th>IRPM</th></tr></thead>
...
<template id="retired-comparativa">
  <div class="chart-header">
    <div>
...
<footer class="site-footer">
  ...
  <div class="footer-right">
    <a href="#metodologia">Metodología</a>
    <a href="https://github.com/mariano-laxague/irpm-argentina" target="_blank">GitHub</a>
...
function buildAnnotations(displayData = [], withHitos = true) {
  ...
  if (withHitos && displayData.length) {
    ...
    // Reading rail: periods are packed left-to-right to avoid overlaps. Their
    // widths, however, follow the portion of each process visible in the
    // current range, so zooming never gives every process the same span.
    ...
    const usableSpan = Math.max(0, lastIndex - gap * (visiblePeriods.length - 1));
    // A small floor keeps labels readable; the remaining span is allocated
    // proportionally to the number of visible observations in each process.
    const minSpan = Math.min(22, Math.max(1, Math.floor(usableSpan / Math.max(visiblePeriods.length, 1))));
    const visibleLengths = visiblePeriods.map(ev => {
      const start = ev.start || ev.date;
      const end = ev.end || ev.date;
      return Math.max(1, displayData.filter(row => row.d >= start && row.d <= end).length);
    });
    const totalVisibleLength = visibleLengths.reduce((sum, length) => sum + length, 0);
    const distributableSpan = Math.max(0, usableSpan - minSpan * visiblePeriods.length);
    let cursor = 0;
    ...
    visiblePeriods.forEach((ev, order) => {
      ...
      const fill = ev.impact === 'positive' ? 'rgba(21,128,61,0.12)'
        ...
        : 'rgba(180,83,9,0.11)';
      const evN = ev.n;
      const extraSpan = totalVisibleLength
        ? Math.floor(distributableSpan * visibleLengths[order] / totalVisibleLength)
        : 0;
      const startIdx = cursor;
      const endIdx = order === visiblePeriods.length - 1
        ? lastIndex
        : Math.min(lastIndex, startIdx + minSpan + extraSpan);
      cursor = endIdx + gap;
...
function refreshAll() {
  ...
  delete iepChart.options.scales.x.min;
  delete iepChart.options.scales.x.max;
...
let rangeData  = [];   // immutable source for the selector; chart data is its visible slice
...
function initRangeSelector() {
  ...
  function updateVisuals() {
    const lastIdx = rangeData.length - 1;
    ...
    const leftDate = rangeData[Math.round(rangeLeft * lastIdx)]?.d;
    const rightDate = rangeData[Math.round(rangeRight * lastIdx)]?.d;
  ...
  function updateRangeLabels(from, to) {
    if (!labels) return;
    const a = rangeData[from];
    const b = rangeData[Math.min(to - 1, rangeData.length - 1)];
  ...
  function onKeydown(event, side) {
    ...
    let next = side === 'left' ? rangeLeft : rangeRight;
    ...
    if (side === 'left') rangeLeft = Math.max(0, Math.min(next, rangeRight - 0.05));
    else rangeRight = Math.min(1, Math.max(next, rangeLeft + 0.05));
    applyRange();
  }
  hLeft.addEventListener('keydown', event => onKeydown(event, 'left'));
  hRight.addEventListener('keydown', event => onKeydown(event, 'right'));
  ...
  // Initial labels
  updateRangeLabels(0, rangeData.length);
  updateVisuals();
}
...
function initDataAlternative() {
  const latest = ALL_DATA[ALL_DATA.length - 1];
  const summary = document.getElementById('data-summary');
  const body = document.getElementById('data-table-body');
  if (!latest || !summary || !body) return;
  summary.textContent = `Última observación: ${latest.d}. IRPM ${latest.v.toFixed(1)}.`;
  body.innerHTML = ALL_DATA.slice(-10).reverse().map(d =>
    `<tr><td>${d.d}</td><td>${d.v.toFixed(1)}</td></tr>`).join('');
  document.getElementById('download-csv')?.addEventListener('click', () => {
    const rows = ['fecha,irpm', ...ALL_DATA.map(d => [d.d, d.v].join(','))];
    const url = URL.createObjectURL(new Blob([rows.join('\n') + '\n'], {type: 'text/csv;charset=utf-8'}));
    const link = Object.assign(document.createElement('a'), {href: url, download: 'irpm_serie.csv'});
